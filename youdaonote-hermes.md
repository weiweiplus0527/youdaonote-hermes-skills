...
有道云笔记不支持 `[[wikilinks]]` 原生双链。笔记间引用使用 Markdown 链接格式：

```
[页面标题](https://note.youdao.com/web/#/file/{fileId})
```

每次创建笔记后，Agent 记录其 fileId。当内容涉及其他已有笔记时，用 `[标题](url)` 格式建立可点击链接。对尚未创建的引用目标，用 `→ 标题` 文本占位，创建后替换为链接。
